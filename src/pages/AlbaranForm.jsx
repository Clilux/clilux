...
export default function AlbaranForm() {
  ...
  const { data: albaran, isLoading } = useQuery({
    queryKey: ['albaran', albaranId],
    queryFn: async () => {
      const items = await base44.entities.Albaran.filter({ id: albaranId });
      const alb = items[0];
      if (alb) setFormData(alb);
      return alb;
    },
    enabled: !!albaranId,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (albaranId) {
        await base44.entities.Albaran.update(albaranId, data);
      } else {
        await base44.entities.Albaran.create(data);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['albaranes'] });
      toast.success(albaranId ? 'Albarán actualizado' : 'Albarán creado');
      navigate(createPageUrl('Albaranes'));
    },
    onError: () => toast.error('Error al guardar el albarán'),
  });
        ...
        <Card className="p-6 bg-white/10 backdrop-blur-sm border-white/20">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <Label className="text-slate-300">Líneas del Albarán</Label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between p-2 rounded bg-emerald-500/20 border border-emerald-500/30">
