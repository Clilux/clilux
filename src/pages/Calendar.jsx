...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, ClipboardCheck, AlertCircle, List } from 'lucide-react';
...
export default function Calendar() {
  ...
  // Vista de agenda - eventos agrupados por mes
        ...
        {viewMode === 'calendar' ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          {dayEvents.some(e => e.type === 'completed') && (
          ...
          <Card className="p-6 bg-white border-0 shadow-sm">
            <div className="space-y-6">
              {Object.keys(agendaEvents).sort().map(monthKey => (
                <div key={monthKey}>
                  <h3 className="text-lg font-semibold text-slate-800 mb-4 capitalize">
                    {format(new Date(monthKey + '-01'), 'MMMM yyyy', { locale: es })}
                  </h3>
                  <div className="space-y-2">
                    {agendaEvents[monthKey].map((event, idx) => {
                      const bgColor = event.type === 'completed' ? 'bg-blue-50 border-blue-100' :
                        event.status === 'on-time' ? 'bg-green-50 border-green-100' :
                        event.status === 'overdue' ? 'bg-yellow-50 border-yellow-100' : 'bg-red-50 border-red-100';
