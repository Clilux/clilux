        ...
        <div className="space-y-4">
            ...
            {filas.length === 0 ? (
              ...
              <div key={nodo.id} className="border-t border-slate-100">
                <div className={`grid grid-cols-2 md:grid-cols-12 gap-2 px-3 py-2 text-sm md:text-base ${esCapitulo(nodo) ? 'bg-slate-50 font-semibold text-slate-800' : ''}`}>
                  <span className="col-span-2 md:col-span-5 min-w-0 truncate" style={{ paddingLeft: `${profundidad * 14}px` }}>
                  ...
                  <span className="md:col-span-1 md:text-right text-slate-500">{nodo.tipo === 'partida' ? euros(margenPartida(nodo).coste) : ''}</span>
                  <span className="md:col-span-1 md:text-right text-slate-600">{nodo.tipo === 'partida' ? euros(nodo.precio) : ''}</span>
                  <span className="md:col-span-1 md:text-right font-medium text-emerald-700">
                    {nodo.tipo === 'partida' && !margenPartida(nodo).sinCoste ? `${margenPartida(nodo).pct} %` : ''}
                  </span>
          ...
          <div className="flex justify-end">
            <div className="w-full md:w-72 space-y-1 text-sm md:text-base">
              ...
              <div className="flex justify-between text-slate-500"><span>Coste estimado</span><span>{margen.sinCoste ? '—' : euros(margen.coste)}</span></div>
              <div className="flex justify-between text-emerald-700 font-medium">
                <span>Margen</span>
                <span>{margen.sinCoste ? '—' : `${margen.pct} % · ${euros(margen.importe)}`}</span>
              </div>
          ...
          <div className="flex flex-wrap justify-end gap-2 pt-1">
            ...
            {onGenerarPedido && (
              <Button variant="outline" className="gap-2" onClick={() => onGenerarPedido(p)}>
                <ShoppingCart className="h-4 w-4" />Generar pedido
              </Button>
            )}
